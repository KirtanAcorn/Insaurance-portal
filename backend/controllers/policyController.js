...
const updateExistingPolicy = async (pool, policyData, existingRecord, res) => {
  try {
    ...
    // Update common company information fields if provided
    ...
    return res.status(201).json({ 
      message: `${policyData.propertyType} policy added successfully to existing company record`,
      updated: true
    });
  } catch (error) {
    console.error('Error updating existing policy:', error);
...
exports.createPolicy = async (req, res) => {
  ...
  try {
    ...
    // If a record exists, check if the specific property type already has data
    if (existingRecord) {
      let propertyTypeExists = false;
      let propertyTypeField = '';
      
      if (policyData.propertyType === 'Commercial') {
        propertyTypeField = 'Commercial Policy';
        propertyTypeExists = existingRecord['Commercial Policy'] && existingRecord['Commercial Policy'].trim() !== '';
      } else if (policyData.propertyType === 'Marine') {
        propertyTypeField = 'Marine';
        propertyTypeExists = existingRecord['Marine'] && existingRecord['Marine'].trim() !== '';
      } else if (policyData.propertyType === 'Property') {
        propertyTypeField = 'Building Insurance';
        propertyTypeExists = existingRecord['Building Insurance'] && existingRecord['Building Insurance'].trim() !== '';
      } else if (policyData.propertyType === 'Fleet') {
        propertyTypeField = 'Fleet Policy';
        propertyTypeExists = existingRecord['Fleet Policy'] && existingRecord['Fleet Policy'].trim() !== '';
      }
      
      if (propertyTypeExists) {
        return res.status(409).json({ 
          error: 'Duplicate policy detected',
          message: `A ${policyData.propertyType} policy for ${policyData.companyName} in year ${policyData.year} already exists. Please use the Edit function to update the existing policy.`
        });
      }
      
      // If the property type doesn't exist, we'll UPDATE the existing record instead of INSERT
      return await updateExistingPolicy(pool, policyData, existingRecord, res);
    }
...
// Get policy details for claims (matches policy management structure)
exports.getPolicyDetailsForClaim = async (req, res) => {
  const { companyName, renewalYear } = req.query;
  if (!companyName || !renewalYear) {
  ...
  try {
    ...
    if (result.recordset.length > 0) {
      const apiData = result.recordset[0];
      ...
      // Transform data to match policy management structure
    ...
    console.error("Error in getPolicyDetailsForClaim:", error);
    return res.status(500).json({ 
      error: "Failed to fetch policy details for claim",
      details: error.message
    });
  }
};
